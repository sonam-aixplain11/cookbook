...
if __name__ == "__main__":
    os.makedirs("docs/experiments/code_kaggle/models/multi_double_agent_llama/results", exist_ok=True)
    ...
    agents = [
        Agent(
            ...
            description="This is a Python Code Executor. Use it run Python commands. Input should be a Python code to solve a data science problem programmatically. The output will be a detailed explanation of what the executed code achieved. When receiving the input code, you should either run it or explain why it cannot be executed. Additionally, ensure the tool prints the outcome!",
            status="onboarded",
            teamId=1,
        ...
        Agent(
            ...
            assets=[],
            description="This is a Python Code Generator. Use it generate Python commands. Input should describe a data science problem that can be solved programmatically. The output will be the Python code that solves the problem.",
            status="onboarded",
            teamId=1,
            llmId="66b2708c6eb5635d1c71f611",
        ),
    ...
    for idx, _ in enumerate(data):
        if data[idx]["image_expected"] is False:
            ...
            for i in range(3):
                if (
                    os.path.exists(f"docs/experiments/code_kaggle/models/multi_double_agent_llama/results/{idx}_0.json")
                    is False
                ):
                    response = TeamAgentService.run(
                        TeamAgentExecuteInput(
                            ...
                            session_id="multi_double",
                            executionParams={"maxIterations": 60},
                        )
                    )

                    print(response.output)
                    with open(f"docs/experiments/code_kaggle/models/multi_double_agent_llama/results/{idx}_0.json", "w") as f:
